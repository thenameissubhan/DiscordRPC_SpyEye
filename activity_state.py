...
def format_state(active_title, proc_name, title_limit=100, reserved_limit=20):
    """
    Constructs the final state string as follows:
      • The active_title is assumed to be of the format:
            <title text> / <TAB NAME and PROCESS NAME>
      • The portion before " / " is taken as the title portion.
           If its length exceeds title_limit, it is truncated by taking the first (title_limit-3)
           characters and appending an ellipsis ("...").
      • The reserved portion (including the delimiter) is forced to exactly reserved_limit characters.
           In our scheme, the delimiter (" / ") is part of the reserved info.
           That is, we allow (reserved_limit - len(delimiter)) characters for the extracted reserved text.
      • If no delimiter is found in active_title, we default the reserved portion to the
           capitalized process name (preceded by the delimiter).

    The final state is the concatenation:
         final_state = <title portion> + <reserved info>
    """
    delimiter = " / "
    # If the active title contains the delimiter, use everything after the last occurrence.
    if delimiter in active_title:
        # Use rsplit so that if there are multiple "/", we take the right-most split.
        title_part, reserved_part = active_title.rsplit(delimiter, 1)
    else:
        title_part = active_title.strip()
        reserved_part = proc_name.capitalize()

    # Truncate the title part if necessary.
    if len(title_part) > title_limit:
        # Reserve three characters for the ellipsis.
        title_part = title_part[: title_limit - 3] + "..."

    # Format the reserved info to exactly reserved_limit characters.
    # (Reserved info will include the delimiter.)
    reserved_content_length = reserved_limit - len(delimiter)
    reserved_part = reserved_part.strip()
    if len(reserved_part) > reserved_content_length:
        reserved_part = reserved_part[:reserved_content_length]
    else:
        # Right-justify so that short strings take up the full space.
        reserved_part = reserved_part.rjust(reserved_content_length)
    final_reserved = (
        delimiter + reserved_part
    )  # This part is exactly reserved_limit characters.

    final_state = title_part + final_reserved
    return final_state


def get_activity_state_and_image(proc_name, active_title):
    """
    Returns the activity state text, details, and image key based on the extracted domain
    from the active window title.

    The expectation is that `active_title` may be very long. This function builds the final state string as follows:
      • If active_title is shorter than 128 characters, it is used as-is.
      • Otherwise, the title portion (before any " / " delimiter) is allowed up to 100 characters
        (truncated with an ellipsis if needed) and a reserved portion for tab/process info is forced to exactly 20 characters.

    Thus, for titles of 128 characters or longer, the final state is exactly 120 characters:
         [truncated title portion (max 100)] + [reserved info (20)]
    """
    ...
    # If the title is short, use it as is.
    if len(active_title) < 128:
        final_state = active_title
    else:
        final_state = format_state(
            active_title, proc_name, title_limit=100, reserved_limit=20
        )

    print(f"DEBUG: final state length = {len(final_state)}, content = {final_state}")
    return final_state, details, image_key
